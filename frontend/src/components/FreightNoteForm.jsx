...
function FreightNoteForm() {
  ...
  const generatePrintableReceipt = (noteData) => {
    // Calcular totais
    const totalWeight = noteData.goods.reduce((sum, good) => sum + (parseFloat(good.weight) || 0), 0);
    const totalDiscount = noteData.goods.reduce((sum, good) => sum + (parseFloat(good.discount) || 0), 0);
    const totalValue = noteData.goods.reduce((sum, good) => sum + (parseFloat(good.value) || 0), 0);
    const finalValue = totalValue - totalDiscount;
    const totalItems = noteData.goods.reduce((sum, good) => sum + (parseInt(good.quantity) || 0), 0);
    const finalWeight = totalWeight - totalDiscount;
    ...
    // Criar HTML para impressão térmica
    const printContent = `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body>
          <div class="receipt">
            ...
            <div class="header">
              ...
              <div class="info-line">
                <span>Data: ${noteData.issueDate ? new Date(noteData.issueDate).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : new Date().toLocaleDateString('pt-BR')}</span>
              </div>
            </div>
